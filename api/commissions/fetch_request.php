...
// 3. Authenticate active session — aligns with app-wide convention ($_SESSION['user_id'] + 'role')
if (!isset($_SESSION['user_id']) || !isset($_SESSION['role'])) {
    echo json_encode(['success' => false, 'message' => 'Unauthorized access. Please log in.']);
    exit;
}

if (strtolower($_SESSION['role']) !== 'artist') {
    echo json_encode(['success' => false, 'message' => 'Only artists can take commission jobs.']);
    exit;
}
...
try {
    ...
    // 1. Resolve artist_id and account_id from session's user_id (stores artist_id for artist role)
    $stmtArtist = $db->prepare('
        SELECT artist_id, account_id
        FROM artist_tbl
        WHERE artist_id = ?
    ');
    $stmtArtist->execute([$_SESSION['user_id']]);
    ...
    $account_id = $artistRow['account_id']; // Reliable account_tbl FK for ownership checks
    ...
    $stmtDuplicate = $db->prepare('
        SELECT request_id
        FROM commission_request_tbl
        WHERE commission_id = ? AND artist_id = ? AND status_id = 2
    ');
